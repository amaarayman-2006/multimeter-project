# Arduino-Based DC Digital Multimeter

![Arduino Multimeter](https://img.shields.io/badge/Arduino-Uno-blue.svg) ![License](https://img.shields.io/badge/license-MIT-green.svg) ![Version](https://img.shields.io/badge/version-1.0-red.svg)

A comprehensive digital multimeter built using Arduino Uno that measures voltage, current and resistance. This project is a part of "Electric and Electronic Measurements" course at Electric Engineering Dept., Faculty of Engineering, Alexandria University.

## Features

- **Voltage Measurement (DC)**: 0-100V with 0.01V resolution
- **Current Measurement**: 0-220mA
- **Resistance Measurement**: 10Ω-1MΩ with auto-ranging
- **16x2 LCD Display**: Real-time readings with backlight

## Bill Of Components

| Component | Specification / Part Number | Quantity |
|-----------|----------------------------|----------|
| Microcontroller | Arduino Uno (ATmega328P) | 1 |
| Display | 16×2 LCD with I2C backpack | 1 |
| Mode selector | 4-position DIP switch | 1 |
| Voltage divider | Resistor network (100V range) | 1 set |
| Shunt resistor | 22Ω (current sense) | 1 |
| Reference resistor | Ohmmeter voltage divider | 1 |
| Breadboard + wires | Standard 830-tie breadboard | 1 |
| Power supply | USB (5V) or 7–12V DC barrel jack | 1 |

## Circuit Schematic

<img src="Schematic.png" alt="Schematic of the circuit" width="700">

## Documentation

You can access the documentation [**here**](Measurement%20Documentation.pdf)

## Team Members

1. Amaar Ayman Mostafa
2. Ahmed Mohamed Othman
